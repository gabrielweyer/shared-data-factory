...
.PARAMETER Location
The Azure location where we'll deploy the resources. This does not matter as we're not planning on using the resources.
...
Can be identical to `LinkedResourceGroupName`.

.PARAMETER SharedFactoryName
The name of the **shared** Data Factory.
...
Can be identical to `SharedResourceGroupName`.

.PARAMETER LinkedFactoryName
The name of the **linked** Data Factory.
...
$linkedRuntimeDeploymentParameters = @{
    ...
    TemplateFile = Join-Path -Path $PSScriptRoot -ChildPath 'linked-integration-runtime.template.json'
    TemplateParameterObject = @{
        location = $Location
